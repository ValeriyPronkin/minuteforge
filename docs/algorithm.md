...
**Модель может придумать.** У каждого поручения сохраняются отметка времени
и та реплика, из которой оно выписано: по цитате обычно видно сразу,
поручение это или пересказ доклада, а к записи возвращаются только в спорных
местах. Реплика ищется механически, по совпадению значимых слов, — здесь
модель не участвует и соврать не может.

**Исполнитель не приписывается наугад.** Фамилия принимается, только если
она прозвучала рядом с самим поручением — в этой реплике или соседней, — либо
исполнитель и есть говорящий. Иначе пункт уходит в раздел «Требуют
уточнения»: пустой адресат заставляет уточнить перед рассылкой, неверный
