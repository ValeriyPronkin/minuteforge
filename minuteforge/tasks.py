...
должен должна должны должно обязан обязаны предлагаю предлагается предстоит
...
приобретите исправьте устраните смотрите берите езжайте побывайте разошлите
""".split())
...
def keep_directives(tasks: Iterable[Task]) -> list[Task]:
    ...
    """
    return [task for task in tasks if not task.quote or is_directive(task.quote)]
