...
|---|---|---|
| `mistral`, 7B | ~5 ГБ | находит явные «подготовьте к среде», пропускает остальное |
| `llama3.1:8b` | ~6 ГБ | заметно лучше держит формат и исполнителей |
| `qwen2.5:14b` | ~10 ГБ | разбирает и неявные поручения, лучше всех с русским |
...
Если видеопамяти не хватает, модель частично уедет в оперативную и станет
отвечать втрое медленнее — работать будет, но ждать придётся долго.
