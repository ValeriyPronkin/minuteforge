...
def test_an_infinitive_order_survives_when_the_request_is_one_sentence_away():
    """«Просьба какая? Всем регионам аналитику разослать.»

    По отдельности поручения не слышно ни в одной фразе: в первой нет дела, во
    второй нет повелительного наклонения. Вместе — поручение, и отсев снимал
    его, потому что смотрел на фразу, а не на окно вокруг неё.
    """
    from minuteforge.tasks import Task, keep_directives

    task = Task(
        what="Разослать аналитику всем регионам",
        quote="Всем регионам вашу аналитику разослать.",
        context="Пётр Игнатьевич, просьба какая? Всем регионам вашу аналитику "
        "разослать. Наверняка будет обратная связь.",
    )
    assert keep_directives([task]) == [task]


def test_an_infinitive_order_survives_after_being_addressed_by_name():
    """«Марина Аркадьевна, подойти более системно» — обращение по

    имени-отчеству и следом инфинитив. Так поручают вслух не реже, чем
    повелительным наклонением, а слова требования в такой фразе нет вовсе.
    """
    from minuteforge.tasks import Task, keep_directives

    task = Task(
        what="Подойти более системно к первопричинам",
        quote="Марина Аркадьевна, подойти более системно с коллегами.",
        context="Марина Аркадьевна, подойти более системно с коллегами. "
        "Разобраться в первопричинах.",
    )
    assert keep_directives([task]) == [task]


def test_an_infinitive_alone_is_not_an_order():
    """Ни просьбы рядом, ни обращения — значит, рассказ о своей работе."""
    from minuteforge.tasks import Task, keep_directives

    task = Task(
        what="Разослать аналитику всем регионам",
        quote="Всем регионам вашу аналитику разослать.",
        context="Мы разослали аналитику ещё в мае. Всем регионам вашу "
        "аналитику разослать.",
    )
    assert keep_directives([task]) == []


def test_a_norm_retold_nearby_does_not_save_a_report():
    """«Должны» и «необходимо» соседней фразой не спасают.

    Ими пересказывают нормы, и стой они в списке прямых просьб — любой пункт
    рядом с изложением документа считался бы поручением. На этом соседстве
    держится главный мусор протокола.
    """
    from minuteforge.tasks import Task, keep_directives

    task = Task(
        what="Перевозить отходы по графику",
        quote="Мы перевозить отходы начали в мае.",
        context="Регионы должны утвердить порядок. Мы перевозить отходы "
        "начали в мае.",
    )
    assert keep_directives([task]) == []
...
def test_the_address_at_the_start_of_a_reply_reaches_every_order_in_it():
    """Обращение звучит один раз, а поручений за ним идёт три подряд.

    «Алексей Вячеславович, просьба какая? Аналитику разослать… давать
    консультации…» — до второго и третьего окно вокруг цитаты уже не
    достаёт, и графа исполнителя пустела на ровном месте.
    """
    from minuteforge.tasks import Task, with_addressee

    task = Task(
        what="Давать консультации в случае обратной связи",
        quote="Наверняка будет обратная связь, давать консультации.",
        context="Всем регионам аналитику разослать. Давать консультации.",
        opening="Алексей Вячеславович, просьба какая?",
    )
    assert with_addressee([task])[0].who == "Алексей Вячеславович"


def test_a_polite_opening_addresses_no_one():
    """«Коллеги,» открывает половину выступлений и не адресует ничего.

    Хуже того: обращение к залу отменяет графу направления, и вежливый
    зачин лишал бы пункт единственного адресата, который у него был.
    """
    from minuteforge.tasks import Task, with_addressee

    task = Task(
        what="Давать консультации",
        quote="Давать консультации.",
        context="Давать консультации.",
        opening="Коллеги, доброе утро.",
    )
    assert with_addressee([task])[0].who == ""
