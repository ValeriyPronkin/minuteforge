...
#: Ведущего окликают весь штаб — «Гамзатбек Ханифович, разрешите», — а
...
def most_addressed(blocks: Sequence[object]) -> str:
    ...
    речь обращением к ведущему — «Хамзатбек Ханифович, ранее отмечали…», — а
...
#: Сколько идёт само совещание, а не к какому дню успеть: «завершим в
#: течение полутора часов», «через десять минут продолжим». Поручений на
#: часы и минуты не дают — их дают на дни, недели и месяцы, — а вот порядок
#: ведения обсуждают постоянно, и «Завершить встречу в течение полутора
#: часов» из-за этого попадало в протокол пунктом со сроком.
#:
#: Точку на часах это правило не задевает — но не потому, что умеет её
#: отличать, а потому, что срок в часах правило :func:`spoken_due` и так не
#: разбирает: «доложить к 14 часам» в графу не попадает вовсе. Это отдельный
#: пробел, и чинить его надо там, а не здесь.
_HOW_LONG = re.compile(r"(?:через|в\s+течени[ие])\s+[^,]*?(?:час|минут)")
...
def due_point(text: str) -> str:
    ...
    """
    due = firm_due(text)
    if any(word in due.lower() for word in _CADENCE) or _HOW_LONG.search(due.lower()):
        return ""
    return due
...
#: и в протоколе стоял пункт «Подтвердить», исполнитель Орлов.
...
def _lowered(text: str) -> str:
    """Со строчной — кроме сокращений вроде ЖКХ, которые пишутся прописными."""
    ...
    *,
    ...
    for position, task in enumerate(tasks, 1):
        ...
        if not source:
            kept.append(task)
            continue
        if due_point(task.due) or due_point(task.what):
            ...
            #
            # Срок смотрится и в самом пункте, не только в графе: модель
            # часто оставляет его внутри формулировки — «получить лицензию
            # и начать эксплуатацию до 1 декабря», — и графа при этом
            # пустая. Доклад себе срока не назначает, где бы тот ни стоял.
            kept.append(task)
            continue
