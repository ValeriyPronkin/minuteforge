"""Протокол по готовой стенограмме без модели — образец для сверки.
    ...
    python scripts/protocol_without_model.py стенограмма.txt 07.09.2026 куда/


Проходит ровно тем же путём, что и приложение: та же отбраковка переклички,
тот же отбор окон по фразам, те же правила исполнителя, региона и срока, та
...
исполнителя, регион, срок и дату срока. Пунктов здесь будет больше, чем в
...
class RuleOnlyClient:
    ...
    делается исполнитель, регион и срок.
    """
...
def main() -> int:
    ...
    print(f"  с регионом:     {sum(1 for t in tasks if t.region)}")
