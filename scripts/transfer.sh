#!/usr/bin/env bash
# РЕЗЕРВНЫЙ ПУТЬ. Обычно код переносится через git push/pull: на офисной машине
# есть доступ к origin, и бандл там не нужен. Этот скрипт остаётся на случай,
# когда origin недоступен — отозвали ключ, закрыли сеть, машина в изоляции.
# Если перенос идёт бандлом, помните: данные ВКС в него не попадают, они под
# запретом в .gitignore и переносятся отдельно (7-Zip или tar, не zip из
# Finder — он мнёт кириллицу в именах файлов).
#
