...
class Meeting:
    ...
    #: Он же, но только присланным файлом — без имён, вычитанных из записи.
    #:
    #: Разделять пришлось после первого же прогона с описью. В people имена
    #: из двух источников: из файла и из самой записи, а вычитанные из записи
    #: приезжают с ошибками распознавания и мусором — «Уважаемый Джумбалат
    #: Хизерович», «Джим Балат Федорович». Для шапки протокола это терпимо:
    #: человек их видит и правит. Для закрытого списка исполнителей — нет:
    #: мусор становился законным исполнителем ровно потому, что стоял в
    #: списке, и список переставал быть закрытым.
    registry: Sequence[Person] | None = None
...
def _protocol(
    ...
    *,
    ...
    known = meeting.registry if meeting.registry is not None else meeting.people
    roster = roster_of(known, units, voices=named.speakers)
    if roster.people:
        logger.info(
