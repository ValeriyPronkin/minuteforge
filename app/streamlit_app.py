...
#: Как список участников называется в папке заседания. То же имя знает
#: collect_people в prepare_meeting.py: из этих файлов всех заседаний
#: собирается накопительный справочник людей.
ROSTER_NAME = "участники.csv"

#: Наше, а не секретарское: прогоны и посчитанное до совещания. Внутрь не
...
def _materials_in(folder: Path) -> list[Path]:
    ...
    Смотрим в саму папку заседания и на этаж вглубь — в любую подпапку,
    кроме наших. Имя подпапки значения не имеет, и это намеренно:
    «secretary» — соглашение раскладки, а заводит папку человек и назовёт её
    как придётся: «Секретарь», «от секретаря», «Материалы». Требовать точного
    написания значит терять материалы молча — хуже этого здесь ничего нет,
    потому что пропажа выглядит как «файлов не прислали».

    У заседаний, разложенных до этой раскладки, присланное лежит прямо в
    папке — потому корень и смотрится наравне с подпапками.
    """
    ...
    try:
        places += [
            ...
            if path.is_dir() and path.name.lower() not in _OURS
        ]
    except OSError:
        # Общий диск отвалился посреди обхода. Материалы — не то, ради чего
        # стоит ронять интерфейс: о недоступности скажет проверка папки.
        return []
    return sorted(
        path
...
with st.sidebar:
    ...
    if materials:
        ...
        st.caption(
            ...
            "Положите тезисы и выгрузку формы в неё саму или в любую "
            "подпапку — название не важно. Из них соберутся список "
    ...
    # Список участников заседания — файл в его папке. Прежде его загружали
    # через интерфейс даже тогда, когда он лежал рядом с записью: выходило
    # два действия там, где довольно нуля, и путались в них все.
    roster_ready = materials_dir / ROSTER_NAME if materials_dir else None
    if roster_ready is not None and not roster_ready.exists():
        roster_ready = None
    ...
    # Собранный из выгрузки список кладётся файлом туда же. Оттуда его берёт
    # и этот разбор, и накопительный справочник людей следующего заседания —
    # ведущие и докладчики в форму регистрации не подают, и узнаются они
    # только по спискам других заседаний.
    #
    # Лежащий не трогаем: его мог поправить человек, а поправленный список
    # точнее собранного из формы.
    built = st.session_state.get("prepared_roster")
    if built and materials_dir is not None and roster_ready is None:
        target = materials_dir / ROSTER_NAME
        try:
            # С BOM: список открывают в Excel, а он в русской локали без
            # метки читает кириллицу как набор значков.
            target.write_text(built, encoding="utf-8-sig")
        except OSError as exc:
            st.caption(
                f"Список участников собран, но сохранить не вышло: "
                f"{exc.strerror or exc}"
            )
        else:
            roster_ready = target
            st.caption(f"Список участников сохранён: `{target.name}` в папке заседания.")

    if roster_ready is not None:
        st.caption(f"Список участников: `{roster_ready.name}` из папки заседания.")
    roster_file = st.file_uploader(
        "Список участников (csv или txt)",
        type=["csv", "txt"],
        help="Нужен, только если файла нет в папке заседания: лежащий там "
        "«участники.csv» берётся сам. ФИО и должности. Список не заменяет "
        "число говорящих — в нём все приглашённые, а голосов в записи "
        "обычно втрое меньше.",
    )
...
3. **Если модель поднята в LM Studio** — у него другой адрес,
   `http://localhost:1234/v1`. Впишите его ниже.
    ...
    if llm_verify_model == SAME:
        llm_verify_model = ""
    # Про окно проверяющую модель спрашивать незачем: проверка шлёт пункт с
    # его окном — сотни токенов, — и в любой умолчательный предел это
    # укладывается. А вот модель эмбеддингов сюда попасть может, и это тот
    # редкий случай, когда поломка не видна вовсе: каждый запрос сорвётся,
    # сорванный запрос оставляет пункт, и проверка молча не сделает ничего.
    if llm_verify_model and is_embedder(llm_verify_model):
        st.error(
            f"«{llm_verify_model}» — модель эмбеддингов, отвечать на вопросы "
            "она не умеет. Проверка сорвётся на каждом пункте и молча "
            "пропустит весь список. Выберите разговорную."
        )
    ...
    #
    # Спрашивать надо про ту модель, которая выбрана в списке. Предел этот
    # не общий для сервера: он задаётся модели при создании, и у собранной
    # своим Modelfile он свой. Пока здесь стояла модель из настроек по
    # умолчанию, выбравший в списке другую видел чужой предел — и слайдер
    # окна ниже вставал по нему же.
    probe = LLMClient(Settings(llm_base_url=address, llm_model=llm_model))
    window, limit = probe.context_window()
...
with st.expander("Что нужно сделать"):
    st.markdown(
        """
...
1. **Выбрать запись** (или готовую стенограмму, если запись уже
   расшифрована). Убедиться, что в боковой панели «Куда сохранять
   результаты» показывает нужное заседание.
2. **Распознать.** Час записи на видеокарте — минуты.
3. **Кто есть кто.** Проверить имена у меток голосов: имя ставится само
   там, где ведущего назвали вслух, остальные — за вами. Метку, которую не
   узнали, оставить пустой.
4. **Заполнить дату заседания.** От неё считаются сроки: без даты «через две
   недели» останется словами, а графа «Срок датой» — пустой.
5. **Собрать протокол.**

**Ничего не переименовывать и никуда не переносить.** Стенограмма заседания,
протокол и поручения ложатся в папку заседания сами; прогон — в `runs`,
каждый в свою папку.
        """
    )
...
Заседание — это папка, и всё по нему лежит в ней. Заседание узнаётся по
своему устройству, а не по названию: приложение само находит его от
выбранного файла и туда же кладёт результат.
...
| глоссарий — `glossary.md` | общая папка, рядом с заседаниями | вы, один раз на все заседания |

Два файла приложение не переписывает никогда — список участников и
стенограмму заседания. Какой расшифровке и какому списку быть главными,
решает человек: ту же запись расшифровывают по нескольку раз, и вторая
бывает хуже первой. Нужна замена — удалите файл, и он соберётся заново.

Списка участников хватает на два дела сразу: по нему подставляются имена
голосов и по нему же проверяется исполнитель в поручениях — кого в списке
нет, того в графу «Исполнитель» не впишут.

Загрузка файлов через боковую панель — запасной путь. Она нужна, только
когда файла нет на этой машине.
        """
    )
...
with st.expander("Скачать себе"):
    ...
    # Текстом — здесь же, а не только рядом с готовым протоколом. Стенограмма
    # это результат первого шага и уходит дальше сама по себе: сверить спорное
    # место или передать в другое подразделение можно, не собирая документ.
    st.download_button(
        "Стенограмма текстом",
        _text_with_head(transcript, doubtful).encode("utf-8"),
        "стенограмма.txt",
        "text/plain",
    )
    st.download_button(
...
# Очерёдность: загруженное руками важнее всего — его загружают, когда
# лежащее не годится. Потом файл заседания, потом собранное из выгрузки
# формы: собранное как раз и ложится в этот файл, так что расходятся они
# только тогда, когда файл поправил человек.
if roster_file is not None:
    roster = read_people(roster_file)
elif roster_ready is not None:
    roster = read_people(roster_ready)
