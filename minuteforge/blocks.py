...
def drop_soundcheck(
    ...
    *,
    ...
    """Убирает перекличку — везде, где она встретилась.

    Возвращает оставшиеся реплики и число убранных.

    Раньше резалось только начало записи: цепочка коротких реплик от первой
    и до первой содержательной. На живом штабе это не работает по двум
    причинам сразу, и обе видны на записи от 7 сентября — 364 реплики, из
    них 169 переклички, а срезалось ноль.

    Первая: цепочка рвётся обо что угодно. Там запись открывается мусором
    распознавания — «КРИЧИТ Я не знаю. Я не знаю», — и фильтр останавливался
    на первой же реплике, не дойдя до переклички вовсе.

    Вторая, и главная: перекличка не кончается в начале. Регионы
    подключаются весь штаб, и «как нас видно, слышно» звучит на третьем часу
    так же, как на первой минуте. Начало записи тут ни при чём — считать
    надо по самой реплике.

    :param keep: чем проверить, не сказано ли в реплике чего-то по делу.
        Перекличка бывает пополам с работой — «Слышно. Иванов, подготовьте
        справку», — и такую реплику терять нельзя. Обычно сюда передаётся
        :func:`minuteforge.tasks.is_directive`; отдельным доводом, а не
        зашито внутрь, чтобы модуль остался независимым от разбора.
    """
    kept: list[Block] = []
    dropped = 0
    for block in blocks:
        if is_soundcheck(block.text) and not (keep and keep(block.text)):
            dropped += 1
            continue
        kept.append(block)
    return kept, dropped
...
class Transcript:
    ...
    #: Свои слова, которыми подсказывали распознаванию. Пусто — подсказок
    #: не было. Хранится, чтобы два прогона можно было сравнить: без этого
    #: непонятно, отчего расшифровка изменилась.
    hints: str = ""
