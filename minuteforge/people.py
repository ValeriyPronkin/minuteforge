...
#: мысль. «Орлов Виктор Павлович, заместитель мэра города Биробиджан,
...
#: Чем человек представляется, начиная свой доклад: «докладывает такой-то»,
#: «это такой-то», «меня зовут». Имя должно стоять ПОСЛЕ такого слова — в
#: этом всё дело. «Да, Джамбулат Хизирович, на связи» устроено наоборот: имя
#: перед словом, и это обращение к ведущему, а не представление себя.
_INTRODUCES = ("докладывает", "доложу", "меня зовут", "это ", "представляюсь")

#: Чем человек открывает собственный доклад. Требуется обязательно, и вот
#: почему: «докладывает такой-то» звучит и у ведущего, когда он передаёт
#: слово, — «У нас на связи Семёнов Алексей Валерьевич, прокомментируйте,
#: пожалуйста». Приветствие эти два случая разделяет: своё выступление
#: открывают «добрый день», чужое объявляют без него.
_GREETS = (
    "добрый день", "доброе утро", "добрый вечер", "здравствуйте",
    "уважаем", "так точно", "приветствую",
)

#: Сколько знаков от начала реплики считать представлением. Дальше идёт
#: доклад, и фамилии в нём чужие: «по данным Росприроднадзора», «как сказал
#: Александр Александрович».
_INTRODUCTION = 220

#: Сколько знаков после слова представления искать имя. Должность бывает
#: длинной — «первый заместитель председателя правительства», — но не
#: бесконечной, а дальше пошёл уже доклад.
_AFTER_THE_WORD = 130


def introduced_speakers(blocks: Sequence["object"]) -> dict[str, Person]:
    """Кто назвал себя сам, открывая доклад.

    Самый надёжный из способов опознать голос: человек, назвавший своё имя,
    в нём не ошибается. Передача слова ведущим на это не похожа — там имя
    звучит в чужой реплике и в чужом падеже, а распознавание коверкает и то
    и другое.

        Добрый день, это первый заместитель председателя правительства
        Ширков Максим Вячеславович.

        Уважаемые участники совещания, докладывает Митхор Зоевсеевич,
        зам. мэра.

    Ищется только в начале реплики. Дальше по тексту «докладывает» звучит
    про третьих лиц, а у ведущего передача слова стоит в конце — это разные
    места, и путать их нельзя: подписать голос именем того, кому он слово
    передал, хуже, чем оставить голос безымянным.
    """
    found: dict[str, Person] = {}
    for block in blocks:
        ...
        if not speaker or speaker in found:
            continue
        person = _introduction(getattr(block, "text", "") or "")
        if person is not None:
            found[speaker] = person
    return found


def _introduction(text: str) -> Person | None:
    """Имя, которым говорящий назвал себя в начале реплики."""
    opening = text[:_INTRODUCTION]
    lowered = opening.lower()
    if not any(greeting in lowered for greeting in _GREETS):
        return None
    for word in _INTRODUCES:
        at = lowered.find(word)
        if at < 0:
            continue
        # Имя ищется после слова представления, а не во всей реплике:
        # «Джамбулат Хизирович, на связи» — обращение, и имя там стоит до.
        tail = opening[at + len(word):at + len(word) + _AFTER_THE_WORD]
...
def suggest_speakers(blocks: Sequence["object"]) -> dict[str, Person]:
    ...
    """
    # Назвавшийся сам — вернее всего, и его догадка не перебивается.
    guesses: dict[str, Person] = dict(introduced_speakers(blocks))
...
def _mentioned_often(name: str, blocks: Sequence["object"]) -> bool:
    ...
    for block in blocks:
        ...
        # человек, что «Ларина Александра Николаевна», а одно случайное
...
def canonical(name: str, people: Iterable[Person]) -> str:
    """Приводит имя к тому, как оно записано в списке участников.

    Три беды лечатся одним списком.

    Порядок слов: «Огородникова Наталья Юрьевна» и «Наталья Юрьевна
    Огородникова» — один человек, а в протоколе он оказывался двумя.

    Ошибки распознавания: отчество на слух берётся плохо, и «Ольга Евгенина
    Белянина» — это Ольга Евгеньевна. Двух совпавших слов из трёх довольно,
    чтобы узнать человека, а третье может быть искажено.

    Неполное имя: в записи говорят «Семенов Алексей», а в протоколе нужно
    как в списке.

    Совпасть должны хотя бы два слова — по одному имени опознавать нельзя,
...
def _stems(name: str) -> set[str]:
    ...
    не признаёт.
    """
    # ё приводится к е: в списке участников пишут «Семёнов», распознавание
    # отдаёт «Семенов», и без этого один человек становится двумя.
    lowered = (name or "").lower().replace("ё", "е")
    return {
        word[:-2] if len(word) > 5 else word
        for word in re.findall(r"\w{3,}", lowered)
    }
