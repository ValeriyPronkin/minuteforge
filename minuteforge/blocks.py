...
def drop_soundcheck(blocks: Sequence[Block]) -> tuple[list[Block], int]:
    ...
    Возвращает оставшиеся реплики и число убранных. Режем только начало: как
    только пошёл разговор, дальше не трогаем — «нас слышно?» посреди доклада
    это уже часть совещания, и выкидывать её нельзя.
    """
    start = 0
    for index, block in enumerate(blocks):
        if is_soundcheck(block.text):
            start = index + 1
            continue
        # Настоящая реплика: дальше всё оставляем как есть.
        break
