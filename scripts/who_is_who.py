...
#: Чем кончается глагол, приставший к имени: «Докладывает Гамзатбек
#: Ханифович», «Выступает Иванов Иван». Правило поиска ФИО видит три слова
...
def surname(name: str) -> str:
    ...
    Порядок слов в живой речи свободный: «Орлов Виктор Павлович» и
    «Давид Эрнестович Мурадян» — одинаково обычные. Отличается фамилия
...
def mentions(text: str, name: str) -> int:
    ...
    Считается по основе, а не по слову целиком: «Мурадян», «Мурадяна» и
    «Мурадяну» — один человек, и точное совпадение их не признаёт. Это и
...
def gather(blocks: Sequence[Block]) -> list[tuple[Person, int, str]]:
    ...
    трижды — «Давид Эрнецович Мурадян», «Мурадян Давид Эрнестович»,
    «Мурадяна Давида».
    """
