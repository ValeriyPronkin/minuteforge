...
def vram_left(torch_module: Any | None = None) -> tuple[float, float] | None:
    """Сколько видеопамяти свободно и сколько её всего, в гигабайтах.

    Нужно затем, что «не хватило видеопамяти» до сих пор объяснялось
    догадками. Своя чистка между шагами есть и работает, но возвращает она
    память **своего** процесса: `torch.cuda.empty_cache()` отдаёт драйверу
    то, что придержал наш распределитель, и ничего не может поделать с
    памятью, занятой соседом. А сосед здесь обычный — Ollama держит языковую
    модель загруженной ещё пять минут после последнего ответа.

    Драйвер же отвечает честно: `mem_get_info` показывает, сколько свободно
    на карте целиком, со всеми чужими процессами. Если перед распознаванием
    свободно два гигабайта из восьми, вопрос «почему уменьшилась порция»
    закрывается одной строкой в журнале, а не рассуждением.
    ...
    except Exception as exc:  # справка о памяти не повод ронять расчёт
        logger.info("Не удалось узнать, сколько видеопамяти свободно: {}", exc)
        return None
...
def recognize(
    ...
    *,
    ...
    # До первого шага, а не после падения: уменьшенная порция ничего не
    # роняет, она молча ухудшает расшифровку, и понять причину потом уже
    # не по чему.
    memory = vram_left()
    if memory is not None:
        free, total = memory
        logger.info("Видеопамяти свободно {:.1f} ГБ из {:.1f}", free, total)
        if free < CRAMPED:
            logger.warning(
                "Свободно меньше {:.0f} ГБ — распознавание пойдёт на "
                "уменьшенной порции, а то и на модели поменьше. {}",
                CRAMPED, FREE_THE_MEMORY,
            )
...
def _transcribe_with_fallback(
            ...
            try:
                ...
                if batch > 1:
                    ...
                    if not result.fallbacks:
                        ...
                        # модель, то есть когда работа уже провалилась. А
                        # уменьшенная порция не проваливает ничего — она
                        # молча ухудшает расшифровку, и цена этому известна:
                        # на записи 27.08 из неё пропал двухминутный доклад,
                        # то есть целый пункт протокола.
                        note = f"{note}. {FREE_THE_MEMORY}"
    ...
    raise RecognitionError(
        f"Не хватило видеопамяти даже для самой мелкой модели. {FREE_THE_MEMORY} "
        f"Последняя ошибка: {last_error}"
    )


#: Чем освободить видеопамять. Совет один и тот же и при уменьшенной порции,
#: и при полном отказе: почти всегда память занята языковой моделью, которую
#: Ollama держит загруженной после прошлого разбора.
#: Сколько видеопамяти нужно, чтобы `large-v3` пошла без уступок. Цифра из
#: наблюдений: сама модель просит около пяти гигабайт, и ещё гигабайт с
#: небольшим уходит на порцию и промежуточные тензоры.
CRAMPED = 6.0

FREE_THE_MEMORY = (
    "Освободите её: закройте Ollama или задайте OLLAMA_KEEP_ALIVE=0, "
    "либо уменьшите batch_size в настройках заранее, "
    "либо считайте на процессоре — device=cpu."
)
