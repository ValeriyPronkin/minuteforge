...
#: назвали.
#:
#: Лицо у глагола любое, и выбирают его не задумываясь. «Переходим к» —
#: от лица совещания, «перехожу к» — от лица докладчика, «предлагаю перейти
#: к» — от лица того, кто ведёт вежливо. Список из одного лица терял доклады
#: на каждой записи: 17.09 — Республику Тыва на неопределённой форме, 10.09 —
#: город Омск на «Дальше перехожу к городу Омск».
_MOVE = (
    "переходим к", "перехожу к", "перейдем к", "перейдём к", "перейду к",
    "перейти к",
    "вернемся к", "вернёмся к", "вернусь к", "вернуться к",
...
#: «Дальше» здесь нет намеренно, хотя «далее» есть. Проверено на записи
#: 17.09: «Дальше, Александр Александрович.» — не переход, а передача слова,
#: и разметка брала название из соседней фразы, начиная разбор Краснодарского
#: края, которого в повестке не было. Книжное «далее» так не употребляют, а
#: разговорное «дальше» — сплошь и рядом. Настоящий переход в этой форме
#: узнаётся по глаголу: «Дальше перехожу к городу Омск».
_QUIET = (
    "далее", "начнем с", "начнём с", "начинаем с", "рассмотрим",
    "продолжаем", "продолжаю",
    "переходим к", "перехожу к", "перейдем к", "перейдём к", "перейду к",
    "перейти к", "вернемся к", "вернёмся к", "вернусь к", "вернуться к",
)
